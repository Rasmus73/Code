...
import com.badlogic.androidgames.mrnom.model.Snake;
import com.badlogic.androidgames.mrnom.model.SnakePart;
import com.badlogic.androidgames.mrnom.model.Stain;
import com.badlogic.androidgames.mrnom.model.World;
...
public class GameScreen extends Screen
{
	...
	private void updateRunning(List<TouchEvent> touchEvents, float deltaTime)
	{
        ...
        for(int ix = 0; ix < len; ix++)
        {
            ...
            {
                if(event.x < 64 && event.y < 64) // check pause button location.
                {
                	Common.playClick();                        
	...
	private void updatePaused(List<TouchEvent> touchEvents)
	{
		...
		for(int ix = 0; ix < len; ix++)
		{
	        ...
	        if(event.type == TouchEvent.TOUCH_UP)
	        {
	        	...
	        	{
	            	if(event.y > 100 && event.y <= 148) // check continue button location
	            	{
	            		Common.playClick();	                	
	                    state = GameState.Running;
	                    return;
	...
	private void updateGameOver(List<TouchEvent> touchEvents)
	{
		...
		for(int ix = 0; ix < len; ix++)
		{
	        ...
	        if(event.type == TouchEvent.TOUCH_UP)
	        {
	        	if(event.x >= 128 && event.x <= 192 && event.y >= 200 && event.y <= 264) // check continue after game over button location
	        	{
	        		Common.playClick();
	                game.setScreen(new MainMenuScreen(game));
	                return;
